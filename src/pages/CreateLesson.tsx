...
export function CreateLesson() {
  ...
  const [step, setStep] = useState<"upload" | "improve" | "preview" | "saving">(
    "upload"
  );
  ...
  const [processedContent, setProcessedContent] =
    useState<ProcessedLesson | null>(null);
  ...
  const [editingImprovement, setEditingImprovement] =
    useState<ImprovementArea | null>(null);
  ...
  const handleProcess = async () => {
    ...
    try {
      ...
      setError(
        err instanceof Error ? err.message : "Failed to process lesson plan"
      );
      setProcessedContent(null);
    } finally {
  ...
  const getFieldValueByPath = useCallback(
    (path: string): any => {
  ...
  const handleApproveImprovement = (
    ...
    const improvement = improvementAreas.find(
  ...
  const isImprovementHandled = useCallback(
    (id: string) => {
  ...
  const getImprovementsForSection = (
    section: "main" | "objectives" | "materials" | "background" | "sections"
  ) => {
  ...
  const getApprovedCount = (
    ...
    return sectionImprovements.filter((imp) => isImprovementHandled(imp.id))
      .length;
  };
          ...
          <div>
            {isProcessing && (
              <div className="flex flex-col justify-center items-center h-full">
                <LoadingProgress progress={progress} step={processingStep} />
              </div>
            )}
          </div>
        </div>
      ...
      <div className="max-w-4xl mx-auto p-6">
        <div className="mb-6">
          ...
          <p className="text-gray-600">
            Our AI has analyzed your lesson plan and identified areas that could
            be improved. Review each suggestion and decide whether to apply it
            to your lesson.
          </p>
        </div>
          ...
          <div className="bg-yellow-50 border border-yellow-200 p-4 rounded-lg flex items-start">
            ...
            <div>
              ...
              <p className="text-yellow-700 text-sm">
                We've identified {improvementAreas.length} areas where your
                lesson activities could be improved. You can apply these
                suggestions or skip them.
              </p>
            </div>
          </div>
        </div>
        ...
        <div className="grid grid-cols-6 gap-4 mb-6">
          <div
            className={`col-span-6 p-4 rounded-lg border ${
              getApprovedCount("sections") === getImprovementsCount("sections")
          ...
          >
            <div className="flex items-center justify-between mb-2">
              ...
              <span className="text-sm">
                {getApprovedCount("sections")}/
                {getImprovementsCount("sections")}
              </span>
            </div>
        ...
        {unhandledImprovements.length > 0 ? (
          ...
          <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-8 text-center">
            ...
            <p className="text-green-700">
              You've addressed all the suggested improvements for your lesson
              plan.
            </p>
          </div>
        )}
        ...
        {improvedFields.length > 0 && (
          <div className="space-y-6 mb-8">
            ...
            {improvedFields.map((field, index) => {
              const improvement = improvementAreas.find(
                (imp) => imp.fieldPath === field.fieldPath
              );
  ...
  if (step === "preview" && processedContent) {
    return (
      <div className="max-w-6xl mx-auto p-6">
        <div className="mb-6">
          <h1 className="text-2xl font-bold text-gray-900 mb-2">
            Preview Your Lesson Plan
          </h1>
          <p className="text-gray-600">
            Review your lesson plan before saving. You can go back to make
        ...
        <div className="flex justify-between">
          <Button variant="outline" onClick={() => setStep("improve")}>
            <ArrowLeft className="h-4 w-4 mr-1" />
            Back to Improvements
          </Button>
      ...
      <div className="max-w-6xl mx-auto p-6 flex justify-center items-center min-h-[60vh]">
        <div className="text-center">
          ...
          <h2 className="text-xl font-semibold text-gray-900 mb-2">
            Saving Your Lesson Plan
          </h2>
          <p className="text-gray-600">
            Please wait while we save your lesson plan...
          </p>
        </div>
      </div>
    );
