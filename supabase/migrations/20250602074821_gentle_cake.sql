...
-- Create RLS policies for teacher_messages (with IF NOT EXISTS to avoid errors)
CREATE POLICY IF NOT EXISTS "Allow public select on teacher_messages"
ON public.teacher_messages FOR SELECT
TO public
USING (true);

CREATE POLICY IF NOT EXISTS "Allow public insert on teacher_messages"
